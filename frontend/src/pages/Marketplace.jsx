...
import {
  ...
  Plus, Search, TrendingUp, ArrowUpRight,
} from "lucide-react";
import {
...
/* -------- Generic CRUD table -------- */
function CrudTable({ endpoint, columns, formFields, title, module, testid, defaults = {} }) {
            ...
            {formFields.map(f => (
              <div key={f.key}>
                ...
                {f.type === "select" ? (
                  ...
                  <Textarea rows={3} className="mt-1" value={form[f.key] || ""} onChange={(e) => setForm(s => ({ ...s, [f.key]: e.target.value }))} />
                ) : (
                  <Input type={f.type || "text"} className="mt-1" value={form[f.key] ?? ""}
...
export default function Marketplace() {
  ...
  const vehicleOpts = []; // filled by user typing
